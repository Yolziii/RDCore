...
describe("Probability", () => {
    ...
    describe("Bernulli method", () => {
        it("Probability of 1", () => {
            ...
            const n = 1; // Бросаем 2 кубика
            const k = 1; // Единица долджна выпасть 2 раза

            assert.equal(round(die.bernulliMethod(p, n, k)), round(1/6));
        });

        it("Probability of 1-1", () => {
            const die:PDie = new PDie(6);
            assert.equal(round(die.bernulliMethod(1/6, 2, 2)), round(1/36));
        });

        it("Probability of 1-1-1", () => {
            const die:PDie = new PDie(6);
            assert.equal(round(die.bernulliMethod(1/6, 3, 3)), round(1/216));
        });

        it("Probability of 1-1-1-1", () => {
            const die:PDie = new PDie(6);
            assert.equal(round(die.bernulliMethod(1/6, 4, 4)), round(1/1296));
        });

        it("Probability of 1-1-1-1-1", () => {
            const die:PDie = new PDie(6);
            assert.equal(round(die.bernulliMethod(1/6, 5, 5)), round(1/7776));
        });
